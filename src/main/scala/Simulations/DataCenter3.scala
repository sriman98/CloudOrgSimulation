...
object DataCenter3:
  ...
  val config = ObtainConfigReference("DATA_CENTER3") match {
    case Some(value) => value
    case None => throw new RuntimeException("Cannot obtain a reference to the config data.")
  }

  //creating a logger class
  val logger = CreateLogger(classOf[DataCenter3])
  ...
  def Start() =
    ...
    val hostPes = List(new PeSimple(config.getLong("DATA_CENTER3.MIPS_CAPACITY")))
    ...
    def createHostPesList(HOSTS_PES: Int) = {
      ...
      (1 to HOSTS_PES).map(_ => pesList.add(new PeSimple(config.getLong("DATA_CENTER3.MIPS_CAPACITY"))))
      pesList
    }
    ...
    def createHostList(HOSTS: Int) = {
      ...
      (1 to HOSTS).map(hostPesList => hostlist.add(new HostSimple(config.getLong("DATA_CENTER3.HOST_RAM"),
        config.getLong("DATA_CENTER3.HOST_STORAGE"),
        config.getLong("DATA_CENTER3.HOST_BW"),
        createHostPesList(config.getInt("DATA_CENTER3.HOST_PES")))
        .setVmScheduler(vmSchedule)
      ))
      hostlist
    }

    val hostList = createHostList(config.getInt("DATA_CENTER3.HOSTS"))
    ...
    def vmAllocate: VmAllocationPolicyAbstract={
      config.getInt("DATA_CENTER3.VM_ALLOCATION") match {
