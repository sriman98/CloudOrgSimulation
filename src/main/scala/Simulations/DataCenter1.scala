...
object DataCenter1:
 ...
 //making sure the config file exists
  val config = ObtainConfigReference("DATA_CENTER1") match {
    case Some(value) => value
    case None => throw new RuntimeException("Cannot obtain a reference to the config data.")
  }

  //creating a logger class
  val logger = CreateLogger(classOf[DataCenter1])
  ...
  def Start() =
    ...
    val hostPes = List(new PeSimple(config.getLong("DATA_CENTER1.MIPS_CAPACITY")))
    ...
    def createHostPesList(HOSTS_PES: Int) = {
      ...
      (1 to HOSTS_PES).map(_ => pesList.add(new PeSimple(config.getLong("DATA_CENTER1.MIPS_CAPACITY"))))
      pesList
    }
    ...
    def createHostList(HOSTS: Int) = {
      ...
      (1 to HOSTS).map(hostPesList => hostlist.add(new HostSimple(config.getLong("DATA_CENTER1.HOST_RAM"),
        config.getLong("DATA_CENTER1.HOST_STORAGE"),
        config.getLong("DATA_CENTER1.HOST_BW"),
        createHostPesList(config.getInt("DATA_CENTER1.HOST_PES")))
        .setVmScheduler(vmSchedule)
      ))
      hostlist
    }

    val hostList = createHostList(config.getInt("DATA_CENTER1.HOSTS"))
    ...
    def vmAllocate: VmAllocationPolicyAbstract={
      config.getInt("DATA_CENTER1.VM_ALLOCATION") match {
...
// Creating the VMS for the hosts
    val vmList = List(
      ...
      new VmSimple(config.getLong("VM5.MIPS_CAPACITY"),config.getLong("VM5.PES"))
        .setRam(config.getLong("VM5.RAM"))
        .setBw(config.getLong("VM5.BW"))
        .setSize(config.getLong("VM5.STORAGE")),
      new VmSimple(config.getLong("VM6.MIPS_CAPACITY"),config.getLong("VM6.PES"))
        .setRam(config.getLong("VM6.RAM"))
        .setBw(config.getLong("VM6.BW"))
        .setSize(config.getLong("VM6.STORAGE")),
      new VmSimple(config.getLong("VM7.MIPS_CAPACITY"),config.getLong("VM7.PES"))
        .setRam(config.getLong("VM7.RAM"))
        .setBw(config.getLong("VM7.BW"))
        .setSize(config.getLong("VM7.STORAGE")),
      new VmSimple(config.getLong("VM8.MIPS_CAPACITY"),config.getLong("VM8.PES"))
        .setRam(config.getLong("VM8.RAM"))
        .setBw(config.getLong("VM8.BW"))
        .setSize(config.getLong("VM8.STORAGE"))
    )
